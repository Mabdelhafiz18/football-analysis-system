...
class OrchestratorService {
  ...
  async _runLivePipeline(matchId, matchInfo) {
    ...
    const sendModelWindow = (packetsToSend) => {
      ...
      const foulTask = foulClientService
        .analyzeWindow(matchId, ndjsonText, currentWindowNumber)
        .then((result) => {
          console.log('[Orchestrator] Foul window ' + currentWindowNumber + ' completed');
          return result;
        })
        .catch((err) => {
          console.warn('[Foul] Window ' + currentWindowNumber + ' failed:', err.message);
          return null;
        });
