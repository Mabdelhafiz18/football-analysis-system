...
class OrchestratorService {
  startProcessing(matchInfo) {
    ...
    console.log('[Orchestrator] Starting live Vision + Tactical + xG + Offside for match ' + matchId);
  ...
  async _runLivePipeline(matchId, matchInfo) {
    ...
    await jobService.updateJobProgress(
      matchId,
      5,
      'Starting Vision + Tactical + xG + Offside live pipeline...'
    );
    ...
    const sendModelWindow = (packetsToSend) => {
      ...
      const offsideTask = offsideClientService
        .analyzeWindow(matchId, ndjsonText, currentWindowNumber)
        .then((result) => {
          console.log('[Orchestrator] Offside window ' + currentWindowNumber + ' completed');
          return result;
        })
        .catch((err) => {
          console.warn('[Offside] Window ' + currentWindowNumber + ' failed:', err.message);
          return null;
        });

      modelTasks.push(tacticalTask);
      modelTasks.push(xgTask);
      modelTasks.push(offsideTask);
    };
      ...
      async (packet) => {
        ...
        if (now - lastProgressUpdate > 3000) {
          ...
          await jobService.updateJobProgress(
            matchId,
            progress,
            'Live Vision + Tactical + xG + Offside... packets: ' + packetsCount + ', windows: ' + windowNumber
          );
    ...
    await jobService.updateJobProgress(
      matchId,
      96,
      'Vision finished. Waiting for Tactical, xG and Offside windows...'
    );
    ...
    await jobService.updateJobProgress(
      matchId,
      98,
      'Vision + Tactical + xG + Offside finished. Packets: ' + packetsCount + ', windows: ' + windowNumber
    );

    await jobService.completeJob(matchId);

    console.log('[Orchestrator] Vision + Tactical + xG + Offside completed for match ' + matchId);
