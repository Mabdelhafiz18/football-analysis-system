...
class FoulClientService {
  constructor() {
    ...
    this.requestTimeoutMs = Number(process.env.FOUL_REQUEST_TIMEOUT_MS || 300000);
  }
  ...
  async _fetchWithTimeout(url, options) {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        throw new Error('Foul request timed out after ' + this.requestTimeoutMs + ' ms');
      }
  ...
  async analyzeClipWindow({
    ...
    try {
      ...
      formData.append(
        'file',
        new Blob([clipBuffer], { type: 'video/mp4' }),
