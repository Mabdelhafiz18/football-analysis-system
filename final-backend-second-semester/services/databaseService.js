...
class DatabaseService {
  ...
  async generateSummaryFromEvents(matchId) {
    ...
    const shotsOnTargetHome = shotsArray.filter(
      s => s.team === 'home' && (s.is_on_target === true || s.outcome === 'goal' || s.outcome === 'saved')
    ).length;
    const shotsOnTargetAway = shotsArray.filter(
      s => s.team === 'away' && (s.is_on_target === true || s.outcome === 'goal' || s.outcome === 'saved')
    ).length;
  ...
  async createMatch(matchInfo) {
    if (this.usePostgres) {
      const result = await postgresPool.query(
        ...
        VALUES ($1, $2, $3, $4, 'processing', 'processing', $5, $6, NOW(), NOW(), NOW())
        RETURNING *`,
        [
          matchInfo.homeTeam,
          matchInfo.awayTeam,
          ...
          matchInfo.video_url || null,
          matchInfo.video_filename || null
        ]
      );
    ...
    throw new Error('PostgreSQL is required to create matches');
  }
}
